...
def get_player_list(request):
    ...
    if request.method == "GET":
        playerSet = (
            Player.objects.all().filter(user=request.user).order_by("game__started_on")
        )
        if playerSet:
        ...
        all_game_players = (
            Player.objects.all().filter(game=game).order_by("user__first_name")
        )
        ...
        spectators = (
            Spectator.objects.all().filter(game=game).order_by("user__first_name")
        )
...
def view_tags(request):
    ...
    if request.method == "GET":
        ...
        unverified_tags = Tag.objects.filter(
            initiator=player, initiator__game=game, receiver__game=game, active=False
        )

        verified_tags = Tag.objects.filter(
            initiator=player, initiator__game=game, receiver__game=game, active=True
        )
...
def stun_tag(request):
    ...
    if request.method == "GET":
        ...
        date = (
            datetime.fromtimestamp(time).replace(tzinfo=pytz.utc).astimezone(tz=eastern)
        )
        ...
        try:
            tag_modifier = Modifier.objects.get(
                faction=player.faction, modifier_type=ModifierType.TAG
            )
        ...
        try:
            tag = Tag.objects.create_tag(
                player, target, date, location, description, tag_modifier_amount
            )
            tag.active = False
            tag.save()
    ...
    if request.method == "GET":
        ...
        try:
            supply_code = SupplyCode.objects.get(
                game=player.game, code=code, claimed_by__isnull=True, active=True
            )
        except ObjectDoesNotExist:
            return notFound(
