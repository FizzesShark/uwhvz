...
def render_player_info(
    request,
    change_code_form=ChangeCodeForm(),
    report_tag_form=ReportTagForm(),
    claim_supply_code_form=ClaimSupplyCodeForm(),
):
    template_name = (
        "mobile/dashboard/player.html"
        if request.user_agent.is_mobile
        else "dashboard/player.html"
    )

    game = most_recent_game()
    participant = request.user.participant(game)
    team_score = sum(
        [p.score() for p in Player.objects.filter(game=game, role=participant.role)]
    )
    ...
    if emails:
        ...
        if (
            not request.user.groups.filter(name="LegacyUsers").exists()
            and not request.user.groups.filter(name="Volunteers").exists()
        ):
...
class PlayerInfoView(View):
    def get(self, request):
        ...
        if (
            not game.is_running
            or not request.user.participant(game)
            or not request.user.participant(game).is_player
        ):
    ...
    def post(self, request):
        ...
        player.save()
        messages.success(
            request, f"Your score is now {'public' if is_score_public else 'private'}."
        )
        return render_player_info(request)
...
class ChangeCodeView(View):
    ...
    def post(self, request):
        ...
        messages.success(
            request,
            f"You have successfully changed your code to {cleaned_data['code']}",
        )
        return redirect("dashboard")
...
class ReportTagView(View):
    ...
    def post(self, request):
        ...
        if cleaned_data["datetime"].replace(
            tzinfo=timezone("Canada/Eastern")
        ) > datetime.utcnow().replace(tzinfo=timezone("Canada/Eastern")):
            report_tag_form.add_error(
                "datetime", "You can't tag someone in the future!"
            )
            return render_player_info(request, report_tag_form=report_tag_form)

        try:
            receiving_player = Player.objects.get(
                code=receiver_code, game=game, active=True
            )
        ...
        try:
            if initiating_player.is_human:
                tag_modifier = Modifier.objects.get(
                    faction=initiating_player.faction, modifier_type=ModifierType.TAG
                )
        ...
        if Tag.objects.filter(
            ...
            tagged_at=cleaned_data["datetime"].replace(
                tzinfo=timezone("Canada/Eastern")
            ),
        ...
        try:
            tag = Tag.objects.create_tag(
                initiating_player,
                receiving_player,
                cleaned_data["datetime"].replace(tzinfo=timezone("Canada/Eastern")),
...
class ClaimSupplyCodeView(View):
    ...
    def post(self, request):
        ...
        try:
            supply_code = SupplyCode.objects.get(
                game=game,
            ...
            claim_supply_code_form.add_error(
                "code", "That supply code does not exist or has already been redeemed."
            )
            return render_player_info(
                request, claim_supply_code_form=claim_supply_code_form
            )
    ...
    def post(self, request):
        ...
        if cd["recipients"] == "All":
            email = Email.objects.create_email(
                f"{subject_set} Message from {request.user.get_full_name()}",
                cd["message"],
                RecipientGroup.ALL,
                game,
                player_made=True,
...
class ZombieTreeView(View):
    ...
    def get(self, request):
        ...
        tags = Tag.objects.filter(
            initiator__game=game, receiver__game=game, type=TagType.KILL, active=True
        )
        ...
        for tag in tags:
            ...
            edges.append(
                {
                    "from": tag.initiator.code,
                    "to": tag.receiver.code,
                }
            )  # 'title': desc}) #This is from when edges used to hold descriptions
