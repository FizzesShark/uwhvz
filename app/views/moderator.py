...
from app.util import (
    moderator_required,
    most_recent_game,
    running_game_required,
    get_game_participants,
    necromancer_required,
)
...
class KillUnsuppliedHumansView(View):
    ...
    def post(self, request):
        ...
        human_players = Player.objects.filter(
            game=game, role=PlayerRole.HUMAN, active=True
        )
...
class GameStartView(View):
    ...
    def post(self, request):
        ...
        if not game_start_form.is_valid():
            messages.error(
                request,
                "There was an error with your request. Check your inputs again!",
            )
            return redirect("manage_game")
        ...
        try:
            start_day = utc.localize(
                datetime(int(cd["year"]), int(cd["month"]), int(cd["day"]))
            )
        except:
            messages.error(request, "That's not a valid day!")
            return redirect("manage_game")
        Game.objects.create_game(
            name=game_title, started_on=start_day, started_by=request.user
        )

        game = most_recent_game()

        if not "Online" in list(
            SignupLocation.objects.filter(game=game).values_list("name", flat=True)
        ):
...
class GameSetView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.error(
                request,
                f'There was an error with the starting of the game "{game.name}"',
            )
            return redirect("manage_game")
...
class GameEndView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.error(
                request, f'There was an error with the ending of the game "{game.name}"'
            )
            return redirect("manage_game")
...
class ManageGameView(View):
    ...
    def post(self, request):
        ...
        if cd["recipients"] == "Self":
            ...
            recipients = list(
                Player.objects.filter(game=game, active=True).values_list(
            ...
            recipients = list(
                Player.objects.filter(
                    game=game, active=True, role=PlayerRole.ZOMBIE
            ...
            recipients = list(
                Player.objects.filter(
                    game=game, active=True, role=PlayerRole.HUMAN
                ).values_list("user__email", flat=True)
            )
            ...
            recipients = list(
                User.objects.filter(
                    player__game=game, is_active=True, groups__name="Volunteers"
                ).values_list("email", flat=True)
            )
            recipients.extend(
                list(
                    User.objects.filter(
                        player__game=game, is_active=True, groups__name="LegacyUsers"
                    ).values_list("email", flat=True)
                )
            )
        ...
        if email_mods:
            recipients.extend(
                list(
                    Moderator.objects.filter(game=game, active=True).values_list(
                        "user__email", flat=True
                    )
                )
            )
        ...
        if email_spectators:
            recipients.extend(
                list(
                    Spectator.objects.filter(game=game, active=True).values_list(
                        "user__email", flat=True
                    )
                )
            )
        ...
        if cd["recipients"] == "All":
            ...
            messages.success(
                request, "You've sent an email to all Legacy and Volunteer Players."
            )
            email = Email.objects.create_email(
                f"{subject_set} {cd['subject']}",
                cd["message"],
                RecipientGroup.VOLUNTEER,
                game,
            )
        if not email_mods:
        ...
        if not email_spectators:
            messages.warning(
                request, "You didn't include spectators on the email sent."
            )
...
class ManageOZView(View):
    ...
    def get(self, request, oz_shuffle_form=OZShuffleForm()):
        game = most_recent_game()
        players = (
            Player.objects.filter(game=game, is_oz=True)
            .exclude(role=PlayerRole.ZOMBIE)
            .order_by("user__first_name")
        )
        forced_oz = Player.objects.filter(game=game, role=PlayerRole.ZOMBIE).order_by(
            "user__first_name"
        )
        max_value = (
            Player.objects.filter(game=most_recent_game())
            .exclude(role=PlayerRole.ZOMBIE)
            .distinct()
            .count()
        )
    ...
    def post(self, request):
        ...
        players = (
            Player.objects.filter(game=game, in_oz_pool=True)
            .exclude(role=PlayerRole.ZOMBIE)
            .order_by("user__first_name")
        )
...
class StunVerificationView(View):
    ...
    def render_stun_verification(self, request):
        ...
        unverified_stuns = Tag.objects.filter(
            initiator__game=game, receiver__game=game, active=False
        )
        questionable_stuns = []

        for tag in unverified_stuns:
            similar_objects = Tag.objects.filter(
...
class ManageStaffView(View):
    ...
    def render_manage_staff(
        self, request, add_mod_form=AddModForm(), add_volunteer_form=AddVolunteerForm()
    ):
        game = most_recent_game()
...
class ManageLegacyView(View):
    ...
    def render_manage_legacy(self, request, add_legacy_form=AddLegacyForm()):
        ...
        for user in User.objects.all().order_by("first_name"):
            if user.legacy_points():
                ...
                if (
                    sum(
                        Legacy.objects.filter(user=user, value__gt=0).values_list(
                            "value", flat=True
                        )
                    )
                    >= points_for_permanent
                ):
    ...
    def post(self, request):
        ...
        legacy_points, legacy_user, legacy_details = (
            int(cleaned_data["legacy_points"]),
            leg_user,
            cleaned_data["legacy_details"],
        )

        if legacy_points < 0 and -1 * (legacy_points) > legacy_user.legacy_points():
            messages.error(
                request,
                f"{legacy_user} does not have {legacy_points} points available to spend!",
            )
            return self.render_manage_legacy(request, add_legacy_form=legacy_form)

        Legacy.objects.create_legacy(
            user=legacy_user, value=legacy_points, details=legacy_details
        )

        if legacy_points > 0:
            messages.success(
                request,
                f"Succesfully gave {legacy_points} tokens to {legacy_user.get_full_name()}.",
            )
        else:
            messages.success(
                request,
                f"{legacy_user.get_full_name()} succesfully spent {abs(legacy_points)} tokens.",
            )
...
class ManageModsView(View):
    ...
    def post(self, request):
        ...
        mod_id = cd["mod"]
        if mod_id in list(
            Moderator.objects.filter(game=game).values_list("id", flat=True)
        ):
...
class ManageVolunteersView(View):
    ...
    def post(self, request):
        ...
        vol_id = cd["volunteer"]
        if vol_id in list(
            User.objects.filter(groups__name="Volunteers").values_list("id", flat=True)
        ):
...
class ManagePlayersView(View):
    ...
    def render_manage_players(
        self,
        request,
        mod_signup_player_form=ModeratorSignupPlayerForm(),
        signup_loc_form=AddSignupForm(),
    ):
        game = most_recent_game()
        ...
        total_stuns = Tag.objects.filter(
            type=TagType.STUN, initiator__game=game, receiver__game=game, active=True
        ).count()
    ...
    def post(self, request, **kwargs):
        ...
        location, email, participant_role = (
            cleaned_data["location"],
            cleaned_data["email"],
            cleaned_data["participant_role"],
        )
...
class GenerateSupplyCodesView(View):
    ...
    def post(self, request):
        ...
        game = most_recent_game()
        supply_code = SupplyCode.objects.create_supply_code(
            game, cd["value"], cd["code"]
        )
...
class ManageShopView(View):
    ...
    def post(self, request):
        ...
        if buyer.shop_score() < int(cd["cost"]):
            messages.error(
                request, f"{buyer} does not have enough points for this purchase!"
            )
            return redirect("manage_shop")

        supply_code = Purchase.objects.create_purchase(
            buyer=buyer, cost=int(cd["cost"]), details=cd["purchase"], game=game
        )
    ...
    def render_email_templates(self, request):
        ...
        return render(
            ...
            self.template_name,
            {
                "game": game,
                "participant": request.user.participant(game),
                "signup_email_form": SignupEmailForm(
                    initial={
                        "signup_email_html": get_text(
                            "app/templates/jinja2/email/signup.html"
                        ),
                        "signup_email_txt": get_text(
                            "app/templates/jinja2/email/signup.txt"
                        ),
                    }
                ),
                "reminder_email_form": ReminderEmailForm(
                    initial={
                        "reminder_email_html": get_text(
                            "app/templates/jinja2/email/signup_reminder.html"
                        ),
                        "reminder_email_txt": get_text(
                            "app/templates/jinja2/email/signup_reminder.txt"
                        ),
                    }
                ),
                "start_email_form": StartEmailForm(
                    initial={
                        "start_email_html": get_text(
                            "app/templates/jinja2/email/game_start.html"
                        ),
                        "start_email_txt": get_text(
                            "app/templates/jinja2/email/game_start.txt"
                        ),
                    }
                ),
            },
    ...
    def post(self, request):
        ...
        if "change_signup" in request.POST:
            ...
            try:
                ...
                f.close()
                SignupEmailForm().fields["signup_email_html"].initial = cd[
                    "signup_email_html"
                ]
                f = open("app/templates/jinja2/email/signup.txt", "w")
                f.write(cd["signup_email_txt"])
                f.close()
                SignupEmailForm().fields["signup_email_txt"].initial = cd[
                    "signup_email_txt"
                ]
            except:
        ...
        if "change_reminder" in request.POST:
            ...
            try:
                ...
                f.close()
                ReminderEmailForm().fields["reminder_email_html"].initial = cd[
                    "reminder_email_html"
                ]
                f = open("app/templates/jinja2/email/signup_reminder.txt", "w")
                f.write(cd["reminder_email_txt"])
                f.close()
                ReminderEmailForm().fields["reminder_email_txt"].initial = cd[
                    "reminder_email_txt"
                ]
            except:
                messages.error(
                    request, "There was an error updating the reminder email."
                )
                return redirect("email_templates")
        ...
        if "change_start" in request.POST:
            ...
            try:
                ...
                f.close()
                StartEmailForm().fields["start_email_html"].initial = cd[
                    "start_email_html"
                ]
                f = open("app/templates/jinja2/email/game_start.txt", "w")
                f.write(cd["start_email_txt"])
                f.close()
                StartEmailForm().fields["start_email_txt"].initial = cd[
                    "start_email_txt"
                ]
            except:
                messages.error(
                    request, "There was an error updating the game start email."
                )
                return redirect("email_templates")
        ...
        if "send_reminder" in request.POST:
            ...
            messages.success(
                request,
                f"Reminder emails sent to {SignupInvite.objects.filter(game=game,used_at__isnull=True).count()} people.",
            )
        if "send_start" in request.POST:
            ...
            messages.success(
                request,
                f"Game start emails sent to {Player.objects.filter(game=game, active=True).count()} players,\
        ...
        for modifier in modifiers:
            if str(modifier.id) + "-remove" in request.POST:
                ...
                if modifier_count > 1:
                    modifier.delete()
                    messages.success(
                        request, f"Successfully deleted modifier {modifier}."
                    )
                    break
                ...
                if modifier.faction.player_set.count() == 0:
                    ...
                    messages.success(
                        request, f"Successfully deleted faction {modifier.faction}."
                    )
                else:
                    messages.error(
                        request,
                        f"Failed to delete modifier and faction {modifier.faction} since players are still a part of it.",
                    )
...
class ManageFactionsView(View):
    ...
    def render_manage_factions(
        self,
        request,
        mod_add_player_to_faction_form=AddPlayerToFactionForm(),
        mod_add_faction_form=AddFactionForm(),
        mod_add_modifier_form=AddModifierForm(),
    ):
        game = most_recent_game()
        human_players = Player.objects.filter(
            game=game, role=PlayerRole.HUMAN, active=True
    ...
    def post(self, request, **kwargs):
        ...
        if "add-faction" in request.POST:
            ...
            if not mod_add_faction_form.is_valid():
                return self.render_manage_factions(
                    request, mod_add_faction_form=mod_add_faction_form
                )

            cleaned_data = mod_add_faction_form.cleaned_data
            name, description, modifier_type, amount = (
                cleaned_data["name"],
                cleaned_data["description"],
                cleaned_data["modifier_type"],
                cleaned_data["amount"],
            )
            # update if possible
            try:
            ...
            try:
                ...
                if faction == "":
                    ...
                    messages.success(
                        request,
                        f"Updated the faction of {player_object.user.get_full_name()} successfully.",
                    )
                else:
