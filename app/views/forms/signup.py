...
class UserSignupForm(forms.Form):
    first_name = forms.CharField(
        label="First name", widget=forms.TextInput(attrs={"class": "ui-input"})
    )

    last_name = forms.CharField(
        label="Last name", widget=forms.TextInput(attrs={"class": "ui-input"})
    )
    ...
    def clean_password2(self):
        ...
        if password1 and password2 and password1 != password2:
            raise forms.ValidationError(
                "The passwords entered do not match.", code="passwords_do_not_match"
            )
        return password2
