...
def get_players():
    return (
        (x.id, f"{str(x)} - {x.shop_score()}")
        for x in Player.objects.filter(game=most_recent_game()).order_by(
            "user__first_name"
        )
    )
...
def get_users():
    return (
        (x.id, f"{x.get_full_name()} - {x.email}")
        for x in User.objects.filter(is_active=True).order_by("first_name")
    )
...
def get_factions():
    ...
    choices.extend(
        [
            (x.id, x)
            for x in Faction.objects.filter(game=most_recent_game()).order_by("name")
        ]
    )
    return choices
...
def get_users_legacy():
    ...
    return (
        (x.id, f"{x.get_full_name()}, {x.email} - {x.legacy_points()}") for x in users
    )
...
class ModeratorSignupPlayerForm(forms.Form):
    ...
    participant_role = EnumField(ParticipantRole, max_length=1).formfield(
        label="Role",
        required=False,
        help_text="If the player role is blank and the game is running, the player will NOT be able to sign up for the game. "
...
class ModMessageForm(forms.Form):
    ...
    recipients = forms.ChoiceField(
        ...
        choices=[
            ...
            ("Humans", "Humans"),
            ("Zombies", "Zombies"),
            ("Volunteers and Legacy", "Volunteers and Legacy"),
            ("Self", "Self"),
        ],
        widget=forms.Select(
            attrs={
    ...
    subject = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={"class": "ui-input", "rows": "1", "placeholder": "Message subject"}
        ),
    )
    ...
    message = forms.CharField(
        ...
        required=True,
        widget=forms.Textarea(
            attrs={"class": "ui-input", "rows": "10", "placeholder": "Your message"}
        ),
    )
...
class GenerateSupplyCodeForm(forms.Form):
    code = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "ui-input", "placeholder": "Leave blank to auto-gen"}
        ),
    )
    value = forms.IntegerField(
        label="Value",
        initial=5,
        widget=forms.TextInput(attrs={"class": "ui-input", "input_type": "number"}),
    )
...
class AddSignupForm(forms.Form):
    location = forms.CharField(
        label="Location",
        widget=forms.TextInput(attrs={"class": "ui-input", "placeholder": "e.g. MC"}),
    )
...
class OZShuffleForm(forms.Form):
    try:
        amount = forms.IntegerField(
            ...
            initial=Player.objects.filter(game=most_recent_game()).distinct().count()
            // 10,
            min_value=0,
            max_value=Player.objects.filter(game=most_recent_game())
...
class GameStartForm(forms.Form):
    name = forms.CharField(
        label="Name",
        widget=forms.TextInput(
            attrs={"class": "ui-input", "placeholder": "e.g. Fall 3100 HvZ"}
        ),
    )
    ...
    day = forms.IntegerField(
        ...
        min_value=0,
        max_value=31,
        help_text="Please don't try to be smart and enter a day that doesn't exist.",
        widget=forms.TextInput(attrs={"class": "ui-input", "input_type": "number"}),
    )
...
class AddLegacyForm(forms.Form):
    ...
    legacy_points = forms.IntegerField(
        ...
        initial=0,
        help_text="This value can be positive or negative. A positive value indicates a gain of tokens, a negative value indicates a loss of tokens.\
