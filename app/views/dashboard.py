...
class IndexView(MobileSupportedView):
    ...
    def get(self, request):
        ...
        return self.mobile_or_desktop(
            request, {"game": game, "signups": signups, "for_js": for_js}
        )
...
class DashboardView(MobileSupportedView):
    ...
    def get(self, request, change_code_form=ChangeCodeForm()):
        ...
        stuns = Tag.objects.filter(
            initiator__user=request.user, type=TagType.STUN, active=True
        ).count()
        kills = Tag.objects.filter(
            initiator__user=request.user, type=TagType.KILL, active=True
        ).count()
        codes = SupplyCode.objects.filter(
            claimed_by__user=request.user, active=True
        ).count()
        ...
        points_accu = sum(
            Legacy.objects.filter(user=request.user, value__gt=0).values_list(
                "value", flat=True
            )
        )
        points_for_permanent = 8
        if game.is_running and (
            request.user.is_superuser
            or (
                request.user.participant(game)
                and request.user.participant(game).is_moderator
            )
        ):
            unverified = Tag.objects.filter(
                initiator__game=game, receiver__game=game, active=False
            ).count()
            if unverified:
                messages.error(
                    request, f"There are {unverified} tags that require verification"
                )
        return self.mobile_or_desktop(
            request,
            {
                ...
                "kills": kills,
                "codes": codes,
                "emails": emails.order_by("-created_at") if emails else None,
                "pytz": pytz,
                "points_accu": points_accu,
                "points_for_permanent": points_for_permanent,
            },
        )
    ...
    def post(self, request):
        ...
        if "pts" in request.POST:
            ...
            Legacy.objects.create_legacy(
                user=request.user,
                value=-1,
                details=f"Started {game} game with 15 points.",
            )
            ...
            player.save()
            Legacy.objects.create_legacy(
                user=request.user, value=-1, details=f"Started {game} game as OZ."
            )
            messages.success(request, "You will now start the game as OZ.")

        legacy_users = Group.objects.get(name="LegacyUsers")
        if request.user.id not in list(
            User.objects.filter(groups__name="LegacyUsers").values_list("id", flat=True)
        ):
...
class PrevGamesView(View):
    ...
    def get(self, request):
        ...
        return self.render_zombie_tree(
            request, prev_games, prev_games.order_by("-created_at").first()
        )
